...
const LEVEL_LABEL: Record<string, string> = { LARGE: "대", MEDIUM: "중", SMALL: "소" };
...
export default function ProductForm({ mode, initial }: { mode: "new" | "edit"; initial?: ProductData }) {
  ...
  const [f, setF] = useState<Record<string, string>>({
    ...
    image1: String(g("image1")), image2: String(g("image2")), image3: String(g("image3")),
    image4: String(g("image4")), image5: String(g("image5")),
  ...
  // 역할별 수당 실시간 합계
  const roleSumLive = ["buzzReward", "chinkuReward", "managerReward", "salesCenterReward", "mgmtCenterReward", "divisionReward", "hqReward"]
    .reduce((a, k) => a + (f[k] === "" ? 0 : Number(f[k])), 0);
  ...
  async function onSubmit(ev: React.FormEvent) {
    ...
    // 역할별 수당 합계 검증
    const roleSum = ["buzzReward", "chinkuReward", "managerReward", "salesCenterReward", "mgmtCenterReward", "divisionReward", "hqReward"]
      .reduce((a, k) => a + num(f[k]), 0);
    if (f.rewardType === "RATE") {
    ...
    const payload: Record<string, unknown> = {
      ...
      categoryId: categoryId ? Number(categoryId) : null, partnerId: f.partnerId ? Number(f.partnerId) : null,
    ...
    setSaving(false);
    if (res.ok) router.push("/master/products");
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <section className="card p-5">
        <h3 className="mb-3 text-sm font-black text-white">기본 정보</h3>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="sm:col-span-2 block">
            <span className="text-xs font-semibold text-slate-400">상품명 *</span>
            <input className={`mt-1 ${inputCls}`} value={f.name} onChange={(e) => set("name")(e.target.value)} placeholder="상품명" />
          </label>
          <label className="block">
            <span className="text-xs font-semibold text-slate-400">수당 유형</span>
            <div className="mt-1 flex gap-2">
              {(["RATE", "FIXED"] as const).map((t) => (
                <button type="button" key={t} onClick={() => set("rewardType")(t)}
                  className={`flex-1 rounded-lg border px-3 py-2 text-sm font-semibold ${f.rewardType === t ? "border-brand-500 bg-brand-600/20 text-white" : "border-line text-slate-400 hover:bg-navy-800"}`}>
          ...
          <label className="block">
            <span className="text-xs font-semibold text-slate-400">판매여부</span>
            <div className="mt-1 flex gap-2">
              <button type="button" onClick={() => setOnSale(true)}
                className={`flex-1 rounded-lg border px-3 py-2 text-sm font-semibold ${onSale ? "border-pos/50 bg-pos/15 text-pos" : "border-line text-slate-400 hover:bg-navy-800"}`}>판매중</button>
              <button type="button" onClick={() => setOnSale(false)}
                className={`flex-1 rounded-lg border px-3 py-2 text-sm font-semibold ${!onSale ? "border-danger/50 bg-danger/15 text-danger" : "border-line text-slate-400 hover:bg-navy-800"}`}>판매중지</button>
            </div>
          </label>
          <label className="block">
            <span className="text-xs font-semibold text-slate-400">판매가 (원)</span>
            <input type="number" className={`mt-1 ${inputCls}`} value={f.salePrice} onChange={(e) => set("salePrice")(e.target.value)} />
          </label>
          <label className="block">
            <span className="text-xs font-semibold text-slate-400">총수당 (원)</span>
            <input type="number" className={`mt-1 ${inputCls}`} value={f.totalAllowance} onChange={(e) => set("totalAllowance")(e.target.value)} />
          </label>
          <label className="block">
            <span className="text-xs font-semibold text-slate-400">계약(영업) 종료일</span>
            <div className="mt-1"><DatePicker variant="dark" value={f.contractEndDate} min={new Date().toLocaleDateString("sv-SE")} onChange={set("contractEndDate")} placeholder="종료일 선택 (상시 판매 시 비움)" /></div>
            <span className="mt-1 block text-[11px] text-slate-500">미지정 시 상시 판매 (마켓에 D-day 미표시)</span>
          </label>
          <label className="block">
            <span className="text-xs font-semibold text-slate-400">설치상품 여부</span>
            <button type="button" onClick={() => setInstallProduct((v) => !v)}
              className={`mt-1 flex w-full items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold ${installProduct ? "border-brand-500 bg-brand-600/20 text-white" : "border-line text-slate-400 hover:bg-navy-800"}`}>
              <span className={`grid h-5 w-5 place-items-center rounded border ${installProduct ? "border-brand-400 bg-brand-500 text-white" : "border-line text-transparent"}`}>✓</span>
          ...
          <div className="sm:col-span-2">
            ...
            <div className="mt-1 flex gap-2">
              <button type="button" onClick={() => setPopular((v) => !v)}
                className={`rounded-lg border px-4 py-2 text-sm font-semibold ${popular ? "border-amber-400 bg-amber-500/20 text-amber-300" : "border-line text-slate-400 hover:bg-navy-800"}`}>
                {popular ? "★ 인기" : "인기"}
              </button>
              <button type="button" onClick={() => setRecommended((v) => !v)}
                className={`rounded-lg border px-4 py-2 text-sm font-semibold ${recommended ? "border-brand-400 bg-brand-600/25 text-brand-200" : "border-line text-slate-400 hover:bg-navy-800"}`}>
                {recommended ? "✓ 추천" : "추천"}
              </button>
            </div>
          </div>
          <div className="sm:col-span-2">
            <span className="text-xs font-semibold text-slate-400">카테고리 (대 → 중 → 소)</span>
          ...
          <label className="block">
            <span className="text-xs font-semibold text-slate-400">파트너사</span>
            <select className={`mt-1 ${inputCls}`} value={f.partnerId} onChange={(e) => set("partnerId")(e.target.value)}>
      ...
      {/* 역할별 수당 */}
      <section className="card p-5">
        <h3 className="mb-1 text-sm font-black text-white">역할별 수당 <span className="text-xs font-medium text-slate-500">(단위: {unit})</span></h3>
        ...
        {(() => {
          ...
          return (
            <p className={`mt-3 rounded-lg px-3 py-2 text-xs font-bold ${okSum ? "bg-pos/10 text-pos ring-1 ring-pos/30" : "bg-danger/10 text-danger ring-1 ring-danger/30"}`}>
      ...
      <section className="card p-5">
        <h3 className="text-sm font-black text-white">상품 이미지 <span className="text-xs font-medium text-slate-500">({images.length}/5)</span></h3>
        ...
        <div
          ...
          className={`rounded-xl border-2 border-dashed p-6 text-center transition-colors ${dragOver ? "border-brand-500 bg-brand-600/10" : "border-line"} ${images.length >= 5 ? "opacity-50" : ""}`}
        >
          <p className="text-sm text-slate-300">이미지를 여기에 드래그 & 드롭</p>
          <p className="mt-0.5 text-xs text-slate-500">또는</p>
          <button type="button" disabled={images.length >= 5 || uploading} onClick={() => fileRef.current?.click()}
            className="mt-2 rounded-lg border border-brand-500 px-4 py-1.5 text-xs font-bold text-brand-400 hover:bg-brand-600/15 disabled:opacity-40">
      ...
      <section className="card p-5">
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="block">
              <span className="text-xs font-semibold text-slate-400">반품/취소 규정</span>
      ...
      <div className="flex justify-end gap-2">
        <button type="button" onClick={() => router.push("/master/products")} className="rounded-xl border border-line px-5 py-2.5 text-sm font-semibold text-slate-300 hover:bg-navy-800">취소</button>
