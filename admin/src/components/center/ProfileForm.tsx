...
function F({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className={`text-xs font-semibold ${ct.fieldLabel}`}>{label}</span><div className="mt-1">{children}</div></label>;
}

/** 센터 마스터 내 정보 수정 — /api/user/me */
export default function ProfileForm() {
  ...
  const inputCls = `w-full rounded-lg border px-3 py-2 text-sm outline-none placeholder:text-amber-200/30 ${ct.input}`;
  ...
  if (status === "loading") return <p className={`text-sm ${ct.cardSub}`}>불러오는 중…</p>;
  if (status === "error") return <p className="text-sm text-red-400">내 정보를 불러오지 못했습니다.</p>;

  return (
    <form onSubmit={onSubmit} className="space-y-5">
      <section className={ct.card}>
        <h3 className={`mb-3 text-sm font-black ${ct.cardHead}`}>계정 <span className={`text-xs font-medium ${ct.note}`}>(변경 불가)</span></h3>
        <div className="grid gap-4 sm:grid-cols-2">
          <F label="아이디"><div className={`rounded-lg border px-3 py-2 text-sm ${ct.roBox}`}>{me?.userId}</div></F>
          <F label="역할"><div className={`rounded-lg border px-3 py-2 text-sm ${ct.roBox}`}>센터 마스터</div></F>
        </div>
      </section>
      ...
      <section className={ct.card}>
        ...
        <div className="mt-4">
          <span className={`text-xs font-semibold ${ct.fieldLabel}`}>주소</span>
          <div className="mt-1 flex gap-2">
            <input className={`${inputCls} w-32`} value={f.zipcode} readOnly placeholder="우편번호" />
            <button type="button" onClick={() => openDaumPostcode((r) => setF((p) => ({ ...p, zipcode: r.zipcode, address: r.address })))} className={`shrink-0 rounded-lg px-3 text-xs font-bold ${ct.outlineBtn}`}>우편번호 검색</button>
          </div>
      ...
      <section className={ct.card}>
        <h3 className={`mb-3 text-sm font-black ${ct.cardHead}`}>정산 계좌</h3>
        <div className="grid gap-4 sm:grid-cols-3">
      ...
      <section className={ct.card}>
        <h3 className={`mb-3 text-sm font-black ${ct.cardHead}`}>비밀번호 변경</h3>
      ...
      <div className="flex justify-end">
        <button type="submit" disabled={saving} className={`rounded-xl px-6 py-2.5 text-sm font-bold disabled:opacity-60 ${ct.primaryBtn}`}>{saving ? "저장 중…" : "변경 저장"}</button>
      </div>
    </form>
  );
}
