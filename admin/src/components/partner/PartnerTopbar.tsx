        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {NAV.map((n) => n.children ? (
            <div key={n.href} className="group relative flex h-16 items-center">
              <button className={`rounded-lg px-3 py-2 text-sm font-semibold transition-colors ${active(n.href) ? activeCls : idleCls}`}>{n.label} ▾</button>
              <div className="invisible absolute left-0 top-full z-40 min-w-[160px] rounded-lg border border-slate-200 bg-white p-1 opacity-0 shadow-xl transition-opacity group-hover:visible group-hover:opacity-100">
                {n.children.map((c) => (
                  <Link key={c.href} href={c.href} className={`block whitespace-nowrap rounded-md px-3 py-2 text-sm font-semibold ${childActive(c.href, n.href) ? activeCls : idleCls}`}>{c.label}</Link>
                ))}
              </div>
            </div>
          ) : (
