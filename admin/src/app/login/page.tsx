  ...
  async function onSubmit(ev: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && res.data?.token) {
        ...
        // 운영(admin.chinbiz.kr)에서는 Domain=.chinbiz.kr 로 저장해 home(chinbiz.kr)과 공유. https 면 Secure.
        const host = window.location.hostname;
        const domainAttr = host.endsWith("chinbiz.kr") ? "; Domain=.chinbiz.kr" : "";
        const secureAttr = window.location.protocol === "https:" ? "; Secure" : "";
        document.cookie = `${TOKEN_KEY}=${token}; path=/${domainAttr}; SameSite=Lax${secureAttr}${remember ? "; max-age=86400" : ""}`;
