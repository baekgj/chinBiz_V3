...
export function clearToken() {
  if (typeof document !== "undefined") {
    // host-only + 공유도메인(.chinbiz.kr) 둘 다 삭제
    document.cookie = `${TOKEN_KEY}=; path=/; max-age=0; SameSite=Lax`;
    if (typeof window !== "undefined" && window.location.hostname.endsWith("chinbiz.kr")) {
      document.cookie = `${TOKEN_KEY}=; path=/; Domain=.chinbiz.kr; max-age=0; SameSite=Lax`;
    }
  }
  if (typeof window !== "undefined") {
